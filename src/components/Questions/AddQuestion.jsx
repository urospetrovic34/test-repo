...
export const AddQuestion = () => {
    ...
    const [questionData, setQuestionData] = useState({text:"",type:"",order:"",company:user.company})
    ...
    if (allQuestions.status === 'success') {
        allQuestions.data.data.data.map((question, index) => orders.push({ "order": question.attributes.order }))
        if(!orderCheck){
            orderCheck = true
        }
        maxOrder = Math.max.apply(Math, orders.map(function(o) { return o.order; }))
    }
        ...
        <div className="panel">
            ...
            <div className="user-panel">
                ...
                <form className="question-form">
                    ...
                    <input type="text" placeholder="Question text" name="text" onChange={handleQuestionDataChange}/>
                    <label htmlFor="">Question type</label>
                    <SelectQuestionType options={questionTypes} handleTypeChange={handleTypeChange}/>
                    <button className="submit-button" onClick={handleAddQuestion}>Save</button>
                </form>
            </div>
        </div>
    )
